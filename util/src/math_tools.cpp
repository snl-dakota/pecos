...
int cardinality_of_total_degree_polynomial(int num_dims, int degree){
  return nchoosek(num_dims+degree,num_dims);
}
					     ...
					     IntMatrix &indices )
{
  ...
  for ( int l = 1; l < level+1; l++ )
	...
	indices.reshape( num_dims, num_indices + level_indices.numRows() );
      for ( int k = 0; k < level_indices.numRows(); k++ )
	{
	  ...
	  if ( weighted_level <= (double)level )
	    {
	      for( int d = 0; d < num_dims; d++ )
		indices(d,num_indices) = level_indices(k,d);
	      num_indices++;
	    }
	}
    }
  indices.reshape( num_dims, num_indices );
}
