...
Real RegressOrthogPolyApproximation::select_best_active_multi_index()
{
  ...
  for (cit=active_mi.begin(); cit!=active_mi.end(); ++cit) {
	...
	sparseIndices[key]    = curr_sparse_ind; // best overall
      }
    }
...
void RegressOrthogPolyApproximation::combine_coefficients()
{
  ...
  case ADD_COMBINE: {
    ...
    for (i=0; i<num_combine; ++i, ++sp_it, ++ec_it, ++eg_it)
...
void RegressOrthogPolyApproximation::combined_to_active()
{
  ...
  if (!combinedSparseIndices.empty()) {
    SharedRegressOrthogPolyApproxData* data_rep
      = (SharedRegressOrthogPolyApproxData*)sharedDataRep;
    sparseIndices[data_rep->activeKey] = combinedSparseIndices;
		  ...
		  RealVector& exp_coeffs_sum, RealMatrix& exp_grads_sum)
{
  ...
  // 2. sparse_ind into original ref_mi terms may be new (not contained in
  //    sparseIndices) -> insert combined index from multi_index_map (reject
  //    duplicates).  This case invalidates expansionCoeff{s,Grads} (see below)
  //    *** TO DO ***
  // 3. sparse_ind into appended mi are new -> insert combined index from
  //    multi_index_map
  StSCIter cit;
  SizetSet combined_sparse_ind = sparse_ind_sum; // copy
  for (cit=sparse_ind.begin(); cit!=sparse_ind.end(); ++cit)
    combined_sparse_ind.insert(multi_index_map[*cit]);

  size_t i, j, index, num_deriv_v,
  ...
  for (i=0, cit=sparse_ind_sum.begin(); cit!=sparse_ind_sum.end(); ++i, ++cit) {
    index = find_index(combined_sparse_ind, *cit);
    if (expansionCoeffFlag)
      combined_exp_coeffs[index] = exp_coeffs_sum[i];
    if (expansionCoeffGradFlag)
      copy_data(exp_grads_sum[i], num_deriv_v, combined_exp_grads[index]);
  }

  for (i=0, cit=sparse_ind.begin(); cit!=sparse_ind.end(); ++i, ++cit) {
    index = find_index(combined_sparse_ind, multi_index_map[*cit]);
    if (expansionCoeffFlag)
      combined_exp_coeffs[index] += coeff * exp_coeffs[i];
    if (expansionCoeffGradFlag) {
      Real*       combined_exp_grad = combined_exp_grads[index];
...
covariance(PolynomialApproximation* poly_approx_2)
{
  ...
  std::map<UShortArray, SizetSet>::iterator sp_it2
    = ropa_2->sparseIndices.find(key);
  if ( (sp_it  == sparseIndices.end()         ||  sp_it->second.empty()) &&
       (sp_it2 == ropa_2->sparseIndices.end() || sp_it2->second.empty()) )
  ...
  const SizetSet&   sparse_ind   = sp_it->second;
  const SizetSet&   sparse_ind_2 = sp_it2->second;
  Real covar = 0.;
  if (same)
    for (i1=1, cit1=++sparse_ind.begin(); cit1!=sparse_ind.end(); ++i1, ++cit1)
      covar += exp_coeffs[i1] * exp_coeffs[i1]
	    *  data_rep->norm_squared(mi[*cit1]);
  else if (sparse_ind.empty()) // mixed: one set is sparse
...
Real RegressOrthogPolyApproximation::run_cross_validation_expansion()
{
  ...
  if (sparseSoln)
    ...
    SizetSet& sparse_ind = sparseIndices[data_rep->activeKey];
    sparse_ind.clear();
...
compressed_sensing( RealMatrix &A, RealMatrix &B )
{
  ...
  if ( expansionCoeffFlag && !multiple_rhs ) {
    if (sparseSoln) // exploit CS sparsity
      ...
      if (sp_it != sparseIndices.end())
	sparseIndices.erase(sp_it);//sp_it->second.clear();
    }
  }
  else {
			     ...
			     IntVector &p,  RealMatrix &vals )
{
  ...
  SharedRegressOrthogPolyApproxData* data_rep
    = (SharedRegressOrthogPolyApproxData*)sharedDataRep;
  copy_data(coefficients.values(),
	    (int)sparseIndices[data_rep->activeKey].size(),
...
update_sparse(Real* dense_coeffs, size_t num_dense_terms)
{
  SharedRegressOrthogPolyApproxData* data_rep
    = (SharedRegressOrthogPolyApproxData*)sharedDataRep;
  SizetSet& sparse_ind = sparseIndices[data_rep->activeKey];// create if needed
