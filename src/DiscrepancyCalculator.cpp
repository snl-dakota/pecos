...
check_multiplicative(Real truth_fn, Real approx_fn, short corr_order)
{
  ...
  //                  by zero FPE.
  return
    ( Pecos::is_small(std::fabs(approx_fn)) ||
      ( corr_order == 0 && Pecos::is_small(std::fabs(truth_fn))) );
}
		     ...
		     short corr_order)
{
  ...
  for (i=0; i<num_fns; ++i)
    if ( Pecos::is_small(std::fabs(approx_fns[i])) ||
	 ( corr_order == 0 && Pecos::is_small(std::fabs(truth_fns[i])) ) )
