					   ...
					   int verbosity )
{
  ...
  if ( ( frob_norm_of_r * frob_norm_of_r ) /
      ...
      if ( verbosity > 0 )
	{
      ...
      Surrogates::svd_solve( A, b, x, singular_values, rank );
  ...
  while ( !done )
      ...
      if ( cg_tol < 0 )
	{
	  ...
	  info = Surrogates::cholesky_solve( newton_step_pos_def_matrix,
      ...
      else
	{
	  ...
	  int iters_taken = 0;
	  info = 
	    Surrogates::conjugate_gradients_solve( newton_step_pos_def_matrix,
				  ...
				  IntVector &ordering )
{
  ...
  while ( !done )
      ...
      if ( Q.numCols() <= num_active_indices )
	{
      ...
      int colinear =
	Surrogates::qr_factorization_update_insert_column( Q, R, A_col,
							   num_active_indices );
      ...
      if ( !colinear )
	{
	  ...
	  Surrogates::substitution_solve(R_new, Atb_sparse, z, Teuchos::TRANS);

	  //Solve Rx = z via back substitution to obtain signal
	  Surrogates::substitution_solve( R_new, z, x_sparse );
#ifdef PRECOMPUTE_GRAMMIAN
					   ...
					   int verbosity )
{
  ...
  while ( !done )
      ...
      for ( int i = 0; i < num_active_indices; i++ )
	{
      ...
      int colinear =
	Surrogates::cholesky_factorization_update_insert_column( A_sparse_prev,
								 U,
								 A_col,
								 num_active_indices,
								 0.0 );
      ...
      if ( !colinear )
	{
	  ...
	  */
	  ...
	  RealMatrix U_new( Teuchos::View, U, num_active_indices+1, 
			    num_active_indices+1, 0, 0 );
	  Surrogates::substitution_solve(U_new, Atb_sparse, z, Teuchos::TRANS);

	  // Solve U'x = z via back substitution to obtain signal
	  Surrogates::substitution_solve(U_new, z, x_sparse, Teuchos::NO_TRANS);
      ...
      else
	{
      ...
      Surrogates::cholesky_inverse(U_new, AtA_sparse_inv, Teuchos::UPPER_TRI);
      ...
      for  ( int n = 0; n < num_validation_sets; n++ )
	{
	  ...
	  int info = Surrogates::cholesky( H, L, Teuchos::LOWER_TRI, true ); //.4/31.6
	  ...
	  if ( info != 0 )
	    ...
	    {
	      Surrogates::cholesky_inverse( L, H_inv, Teuchos::LOWER_TRI ); //.8/31.6s
  ...
  while ( !done )
      ...
      if ( !sign_condition_violated )
	{
	  RealMatrix A_col( Teuchos::View, A, M, 1, 0, 
			    index_to_add );
	  colinear =
	    Surrogates::cholesky_factorization_update_insert_column( A_sparse,
								     U,
								     A_col,
								     num_covariates,
								     delta );
	  ...
	  Surrogates::column_append( A_col, A_sparse );
      ...
      for ( int active_index = 0; active_index < num_covariates; active_index++ )
	{
      ...
      Surrogates::substitution_solve( U_old, s_sparse, w, Teuchos::TRANS );
      Surrogates::substitution_solve( U_old, w, z, Teuchos::NO_TRANS );
      ...
      if ( sign_condition_violated )
	{
	  ...
	  Surrogates::delete_column( index_to_drop, A_sparse );
				   ...
				   bool use_tpn )
{
  ...
  for ( int i = 0; i < metrics.numCols(); i++ ) 
	...
	cv_residuals[n] = residual[n] / ( 1. - H_loo(n,n) );
      ...
      Real tpn = (Real)M / ((Real)(M - (i+1) ) ) * ( 1.+Surrogates::trace( AtA_sparse_inv) );
	     ...
	     int verbosity )
{
  ...
  while ( true )
      ...
      for ( std::set<int>::iterator it=support.begin(); it!=support.end(); ++it )
	{
      ...
      // --------------------------------------- //
      RealVector x_sparse;
      Surrogates::qr_solve( A_sparse, b, x_sparse, Teuchos::NO_TRANS );
